...
def extract_pdf_with_gemini_ocr(doc, api_key, ocr_threshold=50, max_ocr_pages=50):
    """
    ...
    Args:
        ...
        ocr_threshold: 페이지 텍스트가 이 길이 미만이면 OCR 시도
        max_ocr_pages: OCR 처리할 최대 페이지 수 (초과 시 텍스트만 추출)
    """
    ...
    for page_num, page in enumerate(doc):
        ...
        if len(page_text) < ocr_threshold:
            # OCR 페이지 수 제한 확인
            if len(ocr_tasks) >= max_ocr_pages:
                ocr_skipped += 1
                page_results[page_num] = f"[Page {page_num + 1} - OCR 스킵 (제한 초과)]\n{page_text}\n\n"
                continue
    ...
    # 2단계: OCR 병렬 처리 (per-page 타임아웃 30초)
    if ocr_tasks and api_key and OCR_AVAILABLE:
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
                ...
                for future in concurrent.futures.as_completed(future_to_page, timeout=180):
                    try:
                        page_num, text, used = future.result(timeout=30)
                        page_results[page_num] = text
                        if used:
                            ocr_used = True
                    except concurrent.futures.TimeoutError:
                        p_num = future_to_page[future]
                        page_results[p_num] = f"[Page {p_num + 1} - OCR Timeout]\n\n"

        except concurrent.futures.TimeoutError:
            # 전체 OCR 시간 초과 - 남은 페이지 스킵
    ...
    if ocr_skipped > 0:
        final_text += f"[⚠️ OCR 제한 초과: {ocr_skipped}페이지는 OCR 없이 텍스트만 추출됨 (제한: {max_ocr_pages}페이지)]\n\n"
