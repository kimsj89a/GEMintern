...
def generate_material_summary(api_key, model_name, file_context, mode=None):
    ...
    if mode == 'text_organize':
        ...
        system_prompt = (
            "당신은 모든 종류의 비정형 텍스트(공문, 행사 안내문, 회의록, 메모, 채팅 캡처, 발췌문 등)를 표준 서식의 마크다운으로 정리하는 편집자입니다.\n"
            "\n"
            "[핵심 규칙]\n"
            "1. 원문의 사실·수치·날짜·고유명사를 절대로 변경하지 말고 보존하세요. 임의 추가·추측·요약 금지.\n"
            "2. 서문/인사말 없이 바로 본문으로 시작하세요. 결과(마크다운)만 출력합니다.\n"
            "3. 문서 유형(공문/회의록/메모/리스트/대화 등)에 맞춰 자연스러운 구조를 선택하되, 다음 우선순위로 형식화:\n"
            "   - 제목으로 보이는 행(『...』, 「...」, '~ 개최(안)', '~ 결과보고', '~ 안내', 또는 명백히 첫 줄에 단독 제목) → 최상단 `# 제목`\n"
            "   - 섹션/장 구분 → `##` / `###` 헤딩\n"
            "   - 항목 리스트 → `-` 불릿 (중첩 들여쓰기 유지)\n"
            "   - 키-값/메타 정보(일시, 장소, 참석자, 작성자 등) → `- (키) 값` 형식의 불릿 묶음\n"
            "   - 한국식 불릿 위계: `○` → `##` 또는 굵은 단락, `-` → 불릿, `※`/`▷` → 그대로 또는 `>`\n"
            "\n"
            "[표 강제 변환 — 매우 중요]\n"
            "- 다음 패턴이 보이면 **반드시 GFM 마크다운 표(| ... |)**로 복원하세요. 줄 나열 형태 금지.\n"
            "   (1) 시간표/일정 (시간 + 내용 + 비고 등)\n"
            "   (2) 같은 컬럼이 반복되는 항목 (회사/금액/비고, 이름/직책/연락처 등)\n"
            "   (3) 컬럼 헤더가 한 번 등장하고 셀이 줄바꿈으로 분해된 경우\n"
            "   (4) 비교/대조 정보 (현재/변경, A안/B안 등)\n"
            "- 표 작성 규칙:\n"
            "  | 헤더1 | 헤더2 | 헤더3 |\n"
            "  |------|------|------|\n"
            "  | 값 | 값 | 값 |\n"
            "- 분해된 헤더와 분해된 셀을 원문 순서대로 짝지어 복원. 셀이 비어있으면 공란 유지.\n"
            "- 컬럼이 1~2개여도 표 형태가 어울리면 표로 표현.\n"
            "\n"
            "[기타]\n"
            "- 부서명·서명·작성자 라인이 본문 끝에 따로 적혀 있으면 결과 맨 마지막 별도 줄로 배치.\n"
            "- 불필요한 공백·빈 줄·반복은 정리하되, 단락 구분은 유지.\n"
            "- 마크다운 인라인 강조(`**굵게**`, `*기울임*`) 적절히 사용 가능."
        )
        prompt = file_context
    else:
