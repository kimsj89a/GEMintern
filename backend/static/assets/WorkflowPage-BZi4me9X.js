import{u as Ue,r as l,a as d,j as e}from"./index-BLKPI6N5.js";import{s as Qe,u as ae}from"./ws-DJzVGobm.js";import{F as Se}from"./FolderTree-BprDFFL0.js";import{F as Ce}from"./FilePicker-DUZ2lKXX.js";import{C as _e}from"./ChatWidget-Ds6Z7b3h.js";import{M as w}from"./MarkdownViewer-DKrzzGTI.js";import{c as z,d as U,g as I}from"./clipboard-C_2qNmF_.js";import{G as Ke}from"./GenerationProgress-BVnYpJsd.js";import"./index-Bw3ScG1o.js";function qe(r,h){if(!r||!h||r===h)return h;const g=x=>x.split(/\n{2,}/).map(m=>m.trim()).filter(Boolean),u=g(r),S=g(h),v=x=>x.replace(/\s+/g," ").trim(),Q=new Set(u.map(v));return S.map(x=>{const m=v(x);if(Q.has(m))return x;if(/^#{1,6}\s/.test(x)){const K=v(x.replace(/^#{1,6}\s+/,""));if(u.some(le=>v(le.replace(/^#{1,6}\s+/,""))===K))return x}return`<div class="diff-highlight">
...
※ 각 섹션의 내용 분량에 따라 슬라이드 개수가 자동 조절됩니다.`},Me={phase1:{title:"사전 정보 수집",desc:"투자 대상 기업/자산의 기초 자료를 수집하고 분석합니다.",steps:[{id:1,label:"자료 업로드"},{id:2,label:"자료 분석"},{id:3,label:"자료 Q&A"}]},phase2:{title:"문서 작성",desc:"수집된 자료를 바탕으로 보고서 또는 PPT를 생성합니다.",steps:[]}};function ot(){const{currentProject:r,activePage:h}=Ue(),g=Me[h]||Me.phase2,u=h==="phase1",[S,v]=l.useState(1),[Q,x]=l.useState({}),[m,K]=l.useState([]),[le,Te]=l.useState(""),[re,C]=l.useState(!1),[i,j]=l.useState(""),[q,_]=l.useState(""),[be,fe]=l.useState(!1),[J,ne]=l.useState(""),[Ee,Pe]=l.useState(0),[T,E]=l.useState(""),[R,oe]=l.useState([]),[P,X]=l.useState(null),[ie,Y]=l.useState(null),[A,D]=l.useState(!1),[ce,N]=l.useState([]),[ge,f]=l.useState(!1),[$,$e]=l.useState(""),[B,ve]=l.useState(!0),[L,O]=l.useState("choice"),[c,Ie]=l.useState(""),[p,Z]=l.useState([]),[de,Ae]=l.useState(!0),[M,je]=l.useState(""),[y,W]=l.useState(""),[F,V]=l.useState(!1),[H,xe]=l.useState(!1),[Ne,De]=l.useState([]),ue=l.useRef(!1),me=l.useRef(!1),ee=l.useRef(!1),G=l.useRef(!1),he=l.useRef(null),b=l.useRef(null);l.useEffect(()=>{r&&d.getProjectDocs(r).then(t=>x(t.folder_tree||{})).catch(()=>x({}))},[r]);const ye=async t=>{if(r){fe(!0),ne("");try{const s=await d.uploadFiles(r,t),a=Object.keys(s.parsed_texts||{}).length,n=s.parse_errors?.length>0?` (${s.parse_errors.length}개 파싱 실패)`:"";ne(`${a}개 파일 업로드 완료${n}`);const o=await d.getProjectDocs(r);x(o.folder_tree||{})}catch{ne("업로드 실패")}fe(!1)}},we=async()=>{if(!(!r||!M.trim())){V(!0),W(""),xe(!1),me.current=!1;try{const{task_id:t}=await d.startAnalysis({task_type:"web_research",kwargs:{query:M.trim()}}),s=async()=>{if(me.current)return;const a=await d.getTaskStatus(t);a.status==="complete"?(W(a.result||""),V(!1)):a.status==="error"?(W(`오류: ${a.error}`),V(!1)):setTimeout(s,1500)};s()}catch(t){W(`오류: ${t.message}`),V(!1)}}},Be=async()=>{if(!r||!y||H)return;const t=`웹리서치_${M.trim().slice(0,30).replace(/[/\\?%*:|"<>]/g,"_")}`;try{await d.saveResearch({project_name:r,doc_name:t,content:y}),xe(!0),De(a=>[...a,{query:M,result:y,saved:!0}]);const s=await d.getProjectDocs(r);x(s.folder_tree||{})}catch(s){alert(`저장 실패: ${s.message}`)}},Le=()=>{me.current=!0,V(!1)},Oe=async()=>{if(r){D(!0),E(""),oe([]),X(null),Y(null),v(2),ue.current=!1;try{const{task_id:t}=await d.startAnalysis({task_type:"material_summary_batch",kwargs:{project_name:r,selected_docs:m}}),s=async()=>{if(ue.current)return;const a=await d.getTaskStatus(t);if(a.status==="complete"){try{const n=JSON.parse(a.result||"[]");oe(n),n.length>0&&Y(0);const o=n.map(se=>`# 📄 ${se.filename}
