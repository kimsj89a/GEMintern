...
try:
    ...
    DOCAI_ERROR_MSG = "google-cloud-documentai 패키지가 설치되지 않았습니다."
...
def get_docai_status():
    """Document AI 상태 확인 (UI에서 사용)"""
    if DOCAI_AVAILABLE:
        return True, "Google Document AI 사용 가능 (서비스 계정 필요)"
    return False, DOCAI_ERROR_MSG


def extract_pdf_with_document_ai(pdf_bytes, project_id, location, processor_id, credentials_json=None):
    """
    Google Document AI를 사용하여 PDF에서 텍스트 추출

    Args:
        pdf_bytes: PDF 파일의 바이트 데이터
        project_id: GCP 프로젝트 ID
        location: 프로세서 위치 (예: us, eu)
        processor_id: Document AI 프로세서 ID
        credentials_json: 서비스 계정 JSON 문자열 (선택사항)

    Returns:
        추출된 텍스트
    """
    if not DOCAI_AVAILABLE:
        return f"[Document AI 오류: {DOCAI_ERROR_MSG}]"
    ...
    try:
        ...
        # 텍스트 추출
        text_content = "[Google Document AI OCR 적용됨]\n\n"
        ...
        return f"[Document AI 오류: {str(e)}]"


def _get_text_from_layout(layout, full_text):
    """Document AI 레이아웃에서 텍스트 추출"""
...
def process_file_with_document_ai(uploaded_file, project_id, location, processor_id, credentials_json=None):
    """
    업로드된 파일을 Document AI로 처리
    ...
    # Document AI가 지원하는 파일 형식
    supported_types = ['pdf', 'png', 'jpg', 'jpeg', 'gif', 'bmp', 'tiff', 'tif', 'webp']

    if file_type not in supported_types:
        return f"[Document AI는 {file_type} 형식을 지원하지 않습니다. 지원 형식: {', '.join(supported_types)}]"

    try:
        file_bytes = uploaded_file.read()

        # 이미지 파일의 경우 mime_type 설정
        mime_type_map = {
            'pdf': 'application/pdf',
            'png': 'image/png',
            'jpg': 'image/jpeg',
        ...
        # Document AI 호출
        result = extract_file_with_document_ai(
            file_bytes,
            mime_type,
            project_id,
            location,
        ...
        if hasattr(uploaded_file, 'seek'):
            uploaded_file.seek(0)
        return f"[Document AI 처리 오류: {uploaded_file.name} - {str(e)}]"


def extract_file_with_document_ai(file_bytes, mime_type, project_id, location, processor_id, credentials_json=None):
    """
    바이트 데이터를 Document AI로 처리 (내부 함수)
    """
    if not DOCAI_AVAILABLE:
        return f"[Document AI 오류: {DOCAI_ERROR_MSG}]"
